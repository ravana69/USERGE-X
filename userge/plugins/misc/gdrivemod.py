...
class Worker(_GDrive):
    ...
    async def upload(self) -> None:
        ...
        if isinstance(self._output, HttpError):
            ...
            out = f"**Uploaded to GIndex Successfully** __in {m_s} seconds__\n\n{self._output}"
...
@userge.on_cmd("gsetupjsjsjsjs", about={"header": "Setup GDrive Creds"})
...
@userge.on_cmd(
    "gcosnjsjsjsnf",
    about={"header": "Confirm GDrive Setup", "usage": "{tr}gconf [auth token]"},
)
...
@userge.on_cmd("gclejssjjsjsar", about={"header": "Clear GDrive Creds"})
...
@userge.on_cmd("gresjsjsjsset", about={"header": "Reset parent id"})
...
@userge.on_cmd("gempjsjsjzty", about={"header": "Empty the Trash"})
